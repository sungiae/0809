...
<%--
		page 50
		1) 지시자
		   page : <%@ page 속성=값 속성=값....%> 속성은 이미 만들어져 있다
		   include : <%@ include file="파일명"%> : JSP안 특정부분에 다른 JSP를 첨부 (조립식) (7장)
		   tablib (12장)
		   ...
		   page 속성 (JSP파일에 대한 정보를 가지고 있다) => language="java" => 다른 언어도 사용이 가능
		                                            ===============
		                                            info="2021.08.09 수정, 제작:홍길동" => 누가 개발했는지 확인, 수정 날짜
		                                            ...
		                                            ***import는 여러번 사용이 가능
		                                            ...
		                                            buffer => 출력메모리(HTML) => 임시공간 => 8kb
		                                                   => 16kb (포털), 32kb
		                                            autoFlush => 브라우저에서 출력된 HTML을 읽어가면 => buffer를 비운다
		                                               => true
		                                            *** errorPage : 에러 발생 시 어떤 에러가 발생했는지 화면 출력
		                                                        출력할 화면을 지정
		                                                        => 500,404,403,415 한개 파일로 처리
		                                            *** isErrorPage : 톰캣에서 처리 => 나눠서 처리 (web,xml)
		                                            pageEncoding : 현재 사용고 있는 파일의 한글처리                       
...
<%--
		...
		{
			public void -jspInit() ====> 생성자 역할 (초기화)
			{
			}
			public void -jspDestory() ==> 소멸자 역할 (소멸자=> 메모리 해제)
			{
			}
			public void -jspInit() ====> 사용자가 요청한 파일을 보내준다 (JSP코딩)
			{
 ...
 --%>
<!-- HTML코드는 out.write("<html>") out.write를 생략하고 있다 (코드를 쉽게 만들수 있게 도와준다) -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
