...
<%--
	  ...
	  2. 실행과정
	                      톰캣
	     basic1.jsp ===============> 자바파일로 변경 ==========> JVM (가상머신) (컴파일로 class파일 만듬)
	                                 basic_jsp.java          컴파일 => basic1_jsp.class (javac)
	                                 (서블릿 파일)                    |
	                 실행 (ctrl+f11)                                |
	                                                          인터프리터(java)
	                                                            => 출력 (HTML만 출력)
	                 ...
	                 out.println("</html>"); ===> 순수 자바로 만든 파일 => Servlet (Server + Let)
	                                                    let = 가벼운 프로그램 (서버에서 실행되는 가벼운 파일)
	                 => 자바만 가지고 하는 코딩의 단점
	                   => 수정시마다 컴파일 : ex.색상하나만 바꿔도 변경
	                     => 컴파일없이 사용하는 파일 (스크립트)을 제작 => JSP
	                 JSP : 사용이 편리하게 만들어져 있다
	                       기본 Java만 알고 있어도 제작이 가능
	                       호환성이 좋다 (운영체제와 상관없이 실행이 가능)
	                       단점 : 보안에 취약, 확장성이 안좋다 (재사용 불가능) => 재사용하려면 자바의 상속, 포함, 오버라이딩이 있어야한다
	                       ==> (보안, 확장성, 재사용) 추가 => MVC (JSP는 출력만하는 역할 => View)
	                           ============================= 실무
	                 JSP는 class
	                 public class basic1_jsp extends HttpServlet
	                 {
	                 	 ...
	                 	 {
	                 	    // 코딩한 내용이 이 안에 들어간다
	                 	 	// 실행할 HTML을 첨부 => 실제로 메소드안에 내용을 채우는 역할
	                 	 	// 사용자에 무엇을 보여줄 것인지 코딩 => JSP
	                 	 	//   일반유저에 전송할 화면을 만든다
	                 	 	                   ===
	                 ...
	                 => JAVA파일을 만드는 것이 아니라 => _jspService() 메소드를 만들어 주는 역할           
	                    ...
	                    => 프로그램 편리를 위해
	                       ...
	                       => HTML앞에
	                          ...
	                          결론 = <html>
	  3. URL처리
	        http://localhost:8082     /JSProject1/jsp/basic1.jsp => 전체를 URL이라고 말한다
	        ====   ========= ====     ===========================
	        프로토콜   서버IP   서버포트     사용자가 요청한 파일 경로 => URI
	        ======================
	                서버관련
	        => 웹 사이트는 사용자가 요청 할 때 (중복이 없는 내용이 있다) => 파일 ==> URL주소 마지막에 반드시 파일명을 추가              
	  ...
	  5. 스크립트
	     ...
	     <%! %> : 선언식 ==> JSP영역이 메소드안에 코드처리
	                       =======================
	                       멤버변수나 메소드를 제작할 수 없다 ==> <%! 멤버변수, 메소드제작 %> => 사용빈도가 거의 없다
	     <%= %> : 표현식(출력할 때) ==> out.println("출력할 내용물을 설정")
	  6. 내장객체 (중요)
	  7. 표현식
	  8. 액션태그
	  9. 자바빈 (VO)
	  ...
	  2) WAS : JSP를 자바로 변경 => 컴파일 => 실행 => 브라우저에서 읽을 수 있게 HTML로 변환 역할
	     === 톰켓, 레진 ...
	  3) 브라우저로 전송을 하기 위해서 HTML을 출력하는 역할 : 서블릿 (자바), JSP
	  ...
	  =========================  ====================
	     서버 주소                    사용자 요청 파일 (URI)
	         ------------------
	           도메인
	           포트번호 : 80 (생략이 가능)
	  ===> ?는 사용자가 요청한 데이터를 다른 파일로 전송
	       a.jsp?변수=값              
	       a.jsp?변수=값&변수=값
	             ============ 보내는 데이터가 2개이상 &로 구분해서 사용 => 공백이 있으면 안된다
